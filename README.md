...
| `/settings` — **Settings** | Turn job sites on/off, choose the target regions (2-letter country codes, e.g. `ie`, `gb`, `ae`), and configure the eligibility rules (worldwide-remote, sponsorship + relocation, country-bounded veto with work-authorization exemption, work-permit veto) with a live tester. **Test sources now** probes every site live. Saved to `data/settings.yaml` |
...
| `GET` | `/api/settings` | Source toggles, target regions, eligibility rules (defaults when `data/settings.yaml` is absent) |
